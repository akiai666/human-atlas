...
import {assetPath,decodeModelResponse} from './model-download';
...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  try{renderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance'});}catch{onError('无法启动三维查看器，请使用支持并启用 WebGL 的浏览器。');return;}
  ...
  renderer.domElement.setAttribute('aria-label','交互式人体解剖模型。拖动旋转，双指或滚轮缩放，点击结构查看详情。');
  ...
  const loadChunk=async(ci:number)=>{
   const chunk=atlas.chunks[ci],compressed=!!chunk.gzip&&typeof DecompressionStream!=='undefined';const response=await fetch(assetPath(compressed?chunk.gzip!:chunk.url),{signal:abort.signal});const buffer=await decodeModelResponse(response,chunk.bytes,compressed);if(disposed)return;
  ...
  const contextLost=(e:Event)=>{e.preventDefault();onError('设备已暂停三维显示，请重新加载以继续。');};renderer.domElement.addEventListener('webglcontextlost',contextLost);
